...
$PLAN_SETTINGS = [
    'Free' => [
        'max_torrents' => 1,      // Maximum number of active torrents
        'max_size_gb' => 2,       // Maximum size per torrent in GB
        'download_speed' => 1,    // Download speed multiplier
        'needs_validity' => false // Whether the plan expires
    ],
    'Standard' => [
        'max_torrents' => 3,      // Maximum number of active torrents
        'max_size_gb' => 10,      // Maximum size per torrent in GB
        'download_speed' => 2,    // Download speed multiplier
        'needs_validity' => true  // Whether the plan expires
    ],
    'Premium' => [
        'max_torrents' => 10,     // Maximum number of active torrents
        'max_size_gb' => 100,     // Maximum size per torrent in GB
        'download_speed' => 3,    // Download speed multiplier
        'needs_validity' => true  // Whether the plan expires
    ]
];
...
class Authentication {
    ...
    public function __construct($options = []) {
        ...
        $this->defaultDiskSpace = isset($options['defaultDiskSpace']) ? (int)$options['defaultDiskSpace'] : $DEFAULT_DISK_SPACE;
     ...
     * @param int $diskSpace Default disk space in GB
     * @param int $validityDays Default validity period in days
     */
    public function setDefaultPlan($plan, $diskSpace = null, $validityDays = null) {
        ...
        if ($diskSpace !== null) {
            $this->defaultDiskSpace = (int)$diskSpace;
        }
    ...
    public function getCurrentUser() {
        ...
        try {
            ...
            return [
                ...
                'diskSpace' => $userData['DiskSpace'] ?? 2,
     ...
     * @return array Result information with success/error message
     */
    public function signUp($name, $email, $password, $csrfToken, $autoLogin = false) {
        $this->logActivity("Signup attempt for email: $email with verification " . 
            ($this->requireEmailVerification ? "enabled" : "disabled") . 
            " (autoLogin: " . ($autoLogin ? "yes" : "no") . ") " .
            "- Plan: {$this->defaultPlan}, Disk space: {$this->defaultDiskSpace}GB");
            ...
            try {
                ...
                if ($autoLogin && !$this->requireEmailVerification) {
                    ...
                    $planMessage = ($plan === 'Free') ? 
                        ...
                        "Your account has been created with the $plan plan. It will be valid for {$this->defaultValidityDays} days.";
    ...
    /**
     * Check if a user can add a new torrent based on their plan
     * 
    ...
    public function canAddTorrent($userId) {
        ...
        try {
            // Get user email to use as a backup identifier
            $emailStmt = $this->pdo->prepare("SELECT userEmail, Plan, Validity FROM users WHERE id = ?");
            ...
            // Get max torrents allowed for this plan
            $maxTorrents = $this->planSettings[$userPlan]['max_torrents'] ?? 1;
            
            // If plan allows multiple torrents
            if ($maxTorrents > 1) {
                $this->logActivity("canAddTorrent: User $userEmail on $userPlan plan can add up to $maxTorrents torrents");
                return [
                    'allowed' => true, 
                    'message' => "$userPlan plan can add up to $maxTorrents torrents",
                    'plan' => $userPlan,
                    'valid' => $isValid,
            ...
            // For restricted plans, check active torrents count
            try {
                $stmt = $this->pdo->prepare("SELECT COUNT(*) as count FROM usertorrents WHERE userEmail = ?");
                $stmt->execute([$userEmail]);
                $result = $stmt->fetch();
                
                $torrentCount = $result['count'] ?? 0;
                $this->logActivity("canAddTorrent: User $userEmail has $torrentCount active torrents (max: $maxTorrents)");

                if ($torrentCount >= $maxTorrents) {
                    return [
                        'allowed' => false, 
                        'message' => "$userPlan users can only have $maxTorrents active torrent at a time. Please delete an existing torrent before adding a new one.",
                        'plan' => $userPlan,
                        'count' => $torrentCount,
                        'maxTorrents' => $maxTorrents
                    ];
                ...
                // If error is due to missing table, log it but continue with allowing
                if (strpos($e->getMessage(), "Base table or view not found") !== false) {
                    $this->logActivity("canAddTorrent: usertorrents table not found, assuming no existing torrents", "warning");
                } else {
    ...
    public function checkTorrentSizeLimit($torrentSize, $userId) {
        ...
        try {
            ...
            if ($torrentSizeGB > $maxSizeGB) {
                ...
                return [
                    'allowed' => false,
                    'message' => "{$userPlan} users are limited to torrents of {$maxSizeGB}GB or less. Please upgrade to download larger files.",
                    'maxSize' => $maxSizeGB,
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && basename($_SERVER['PHP_SELF']) == basename(__FILE__)) {
    ...
    // Create Authentication instance with verification setting
    ...
    try {
        ...
        if (isset($_POST['signUp'])) {
            ...
            $response = $auth->signUp($name, $email, $password, $csrfToken, $autoLogin);
        }
